...
const CAT_GRADIENTS: { [key: string]: string } = {
  'Волосы': 'linear-gradient(135deg, rgba(201,168,76,.18) 0%, rgba(154,114,48,.08) 100%)',
  'Ногти': 'linear-gradient(135deg, rgba(244,163,193,.25) 0%, rgba(236,72,153,.08) 100%)',
  'Лицо':  'linear-gradient(135deg, rgba(134,239,172,.22) 0%, rgba(74,222,128,.08) 100%)',
  'Тело':  'linear-gradient(135deg, rgba(147,197,253,.22) 0%, rgba(96,165,250,.08) 100%)',
};
...
function matchesCategory(svc: Service, cat: string): boolean {
  if (cat === 'Все') return true;
  const catField = (svc.category ?? '').toLowerCase();
  return catField.includes(cat.toLowerCase());
}
...
export default function CatalogPage() {
  ...
  const filtered = services.filter(svc => {
    ...
    const matchCat = matchesCategory(svc, activeCat);
    ...
    <div style={{
      ...
      <div style={{ padding: '18px 22px 10px' }}>
        ...
        <div style={{ fontFamily: SERIF, fontSize: 36, fontWeight: 500, color: NEAR_BLACK, lineHeight: 1.05, marginTop: 10, letterSpacing: '-0.02em' }}>
          Коллекция<br />
          <span style={{ fontStyle: 'italic', color: GOLD }}>услуг</span>.
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px 22px' }}>
          ...
          <div style={{ fontFamily: SERIF, fontSize: 20, fontWeight: 500, color: NEAR_BLACK, marginTop: 12 }}>Услуги не найдены</div>
          <div style={{ fontSize: 13, color: MUTED, marginTop: 6 }}>Попробуйте другой запрос</div>
        </div>
      ) : (
