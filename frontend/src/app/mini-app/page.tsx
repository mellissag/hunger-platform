...
interface SalonInfo { name: string; description: string; address: string; phone: string; }
...
const T: Record<Lang, Record<string, string>> = {
  ru: {
    ...
    book: "Записаться", bookSub: "Выбрать услугу и время",
    myBookings: "Мои записи", myBookingsSub: "Нет записей",
    ai: "AI-консультант", aiSub: "Подбор услуги",
    about: "О салоне", aboutSub: "Адрес и часы работы",
    profile: "Мой профиль", profileSub: "Данные и история визитов",
    settings: "Настройки",
    upcoming: "Предстоящие", completed: "Завершённые",
    noUpcoming: "Нет предстоящих записей", noHistory: "История пуста",
    ...
    loading: "Загрузка…", noServices: "Нет услуг",
    confirmed: "Подтверждена", pending: "Ожидание",
    cancelled: "Отменена", done: "Завершена",
    visits: "Визитов", reschedule: "Перенести",
    historyMenu: "История визитов", editData: "Редактировать данные",
    langTitle: "Язык интерфейса", themeTitle: "Оформление",
  },
  en: {
    welcome: "Welcome",
    book: "Book appointment", bookSub: "Choose service & time",
    myBookings: "My bookings", myBookingsSub: "No bookings",
    ai: "AI consultant", aiSub: "Service recommendation",
    about: "About salon", aboutSub: "Address & working hours",
    profile: "My profile", profileSub: "Data & visit history",
    settings: "Settings",
    upcoming: "Upcoming", completed: "Completed",
    noUpcoming: "No upcoming bookings", noHistory: "History is empty",
    ...
    loading: "Loading…", noServices: "No services",
    confirmed: "Confirmed", pending: "Pending",
    cancelled: "Cancelled", done: "Completed",
    visits: "Visits", reschedule: "Reschedule",
    historyMenu: "Visit history", editData: "Edit details",
    langTitle: "Interface language", themeTitle: "Appearance",
  },
  uk: {
    welcome: "Ласкаво просимо",
    book: "Записатися", bookSub: "Обрати послугу та час",
    myBookings: "Мої записи", myBookingsSub: "Немає записів",
    ai: "AI-консультант", aiSub: "Підбір послуги",
    about: "Про салон", aboutSub: "Адреса та години роботи",
    profile: "Мій профіль", profileSub: "Дані та історія візитів",
    settings: "Налаштування",
    upcoming: "Майбутні", completed: "Завершені",
    noUpcoming: "Немає майбутніх записів", noHistory: "Історія порожня",
    ...
    loading: "Завантаження…", noServices: "Немає послуг",
    confirmed: "Підтверджено", pending: "Очікування",
    cancelled: "Скасовано", done: "Завершено",
    visits: "Візитів", reschedule: "Перенести",
    historyMenu: "Історія візитів", editData: "Редагувати дані",
    langTitle: "Мова інтерфейсу", themeTitle: "Оформлення",
  },
  bg: {
    welcome: "Добре дошли",
    book: "Записване", bookSub: "Изберете услуга и час",
    myBookings: "Моите записи", myBookingsSub: "Няма записи",
    ai: "AI-консултант", aiSub: "Подбор на услуга",
    about: "За салона", aboutSub: "Адрес и работно време",
    profile: "Моят профил", profileSub: "Данни и история",
    settings: "Настройки",
    upcoming: "Предстоящи", completed: "Завършени",
    noUpcoming: "Няма предстоящи записи", noHistory: "Историята е празна",
    ...
    theme: "Тема", language: "Език",
    dark: "Тъмна", light: "Светла",
    bookCta: "Изберете услуга и час",
    loading: "Зареждане…", noServices: "Няма услуги",
    confirmed: "Потвърдено", pending: "Изчакване",
    cancelled: "Отменено", done: "Завършено",
    visits: "Визити", reschedule: "Пренасрочване",
    historyMenu: "История на визитите", editData: "Редактиране на данни",
    langTitle: "Език на интерфейса", themeTitle: "Оформление",
  },
};
...
function twa()       { if (typeof window === "undefined") return null; return window.Telegram?.WebApp ?? null; }
...
const IC = {
  ...
  sun:      <svg viewBox="0 0 24 24" width={16} height={16} fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>,
  moon:     <svg viewBox="0 0 24 24" width={16} height={16} fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>,
  globe:    <svg viewBox="0 0 24 24" width={16} height={16} fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15 15 0 010 20"/></svg>,
  check:    <svg viewBox="0 0 24 24" width={14} height={14} fill="none" stroke="#080808" strokeWidth={2.5} strokeLinecap="round"><polyline points="20,6 9,17 4,12"/></svg>,
};
...
export default function MiniAppHome() {
  ...
  /* ── Persist preferences ── */
  useEffect(() => {
    try { localStorage.setItem("hb_prefs", JSON.stringify({ theme, lang })); } catch { /* ignore */ }
  }, [theme, lang]);

  /* ── Apply CSS vars to :root on theme change ── */
  useEffect(() => {
    const vars = theme === "dark" ? DARK_VARS : LIGHT_VARS;
    const rules = vars.trim().split(";").filter(Boolean).map(r => r.trim());
  ...
  useEffect(() => {
    fetch(`${API}/api/v1/mini-app/services`, {
      ...
      .then((data: Service[]) => {
        ...
        data.forEach(s => { if (s.category_id) catMap.set(s.category_id, pickI18n(s.category_name_i18n, lang)); });
  ...
  const ThemeBtn = () => (
    <button onClick={() => { haptic(); setTheme(t => t === "dark" ? "light" : "dark"); }} style={{
      position: "absolute", top: 12, right: 12,
      width: 32, height: 32, borderRadius: 2, border: "1px solid var(--border)",
      background: "var(--dim)", display: "grid", placeItems: "center",
      cursor: "pointer", color: "var(--gold)", flexShrink: 0, zIndex: 10,
    }}>
  ...
  const TopNav = ({ title }: { title: string }) => (
    <div style={{ display: "flex", alignItems: "center", padding: "10px 14px", gap: 10, background: "var(--card)", borderBottom: "1px solid var(--border)", position: "relative" }}>
      <div className="serif" style={{ flex: 1, fontSize: 17, fontWeight: 600, color: "var(--fg)" }}>{title}</div>
      <ThemeBtn />
    </div>
  );
  ...
  if (screen === "menu") return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div style={{ padding: "20px 18px 16px", background: "var(--card)", borderBottom: "1px solid var(--border)", position: "relative" }}>
        ...
        <div style={{ fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 4 }}>{t("welcome")}</div>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, color: "var(--fg)", paddingRight: 44 }}>{firstName || "Hunger Beauty"}</div>
      ...
      <div style={{ flex: 1, padding: 12, display: "flex", flexDirection: "column", gap: 7 }}>
        {[
          { icon: IC.book,     key: "book",       sub: upcoming.length > 0 ? fmtDate(upcoming[0]!.starts_at) : t("bookSub"), action: () => { haptic(); setScreen("services"); } },
          { icon: IC.list,     key: "myBookings", sub: upcoming.length > 0 ? `${upcoming.length}` : t("myBookingsSub"), action: () => { haptic(); setScreen("bookings"); }, badge: upcoming.length || undefined },
          { icon: IC.ai,       key: "ai",         sub: t("aiSub"),    action: () => { haptic(); setScreen("ai"); } },
          { icon: IC.info,     key: "about",      sub: t("aboutSub"), action: () => { haptic(); setScreen("about"); } },
          { icon: IC.user,     key: "profile",    sub: t("profileSub"), action: () => { haptic(); setScreen("profile"); } },
          ...
          <button key={key} onClick={action} style={{
            ...
            <div style={{ width: 36, height: 36, borderRadius: 2, display: "grid", placeItems: "center", flexShrink: 0, background: "var(--dim)", color: "var(--gold)" }}>{icon}</div>
            <div style={{ flex: 1 }}>
      ...
      {/* Settings shortcut */}
      <button onClick={() => { haptic(); setScreen("settings"); }} style={{
  ...
  if (screen === "services") return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 7 }}>
        {loadingSvc && <div style={{ textAlign: "center", padding: 40, color: "var(--muted)", fontSize: 13 }}>{t("loading")}</div>}
        {filtered.map(svc => (
          <button key={svc.id} onClick={() => { haptic(); goBook(svc.id); }} style={{
            display: "flex", alignItems: "center", gap: 12, padding: "13px 14px",
            borderRadius: 2, cursor: "pointer", background: "var(--card)", border: "1px solid var(--border)", width: "100%", textAlign: "left",
          }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, marginTop: 2, color: "var(--muted)" }}>{fmtDur(svc.duration_minutes)} · {svc.masters_count}</div>
            </div>
      ...
      <div style={{ display: "flex", background: "var(--card)", borderBottom: "1px solid var(--border)" }}>
        {(["upcoming","past"] as const).map(tab => (
          <button key={tab} onClick={() => setBookingsTab(tab)} style={{
            ...
            background: "transparent", color: bookingsTab === tab ? "var(--gold)" : "var(--muted)",
          }}>{tab === "upcoming" ? t("upcoming") : t("completed")}</button>
        ))}
      </div>
          ...
          return (
            <div key={b.id} style={{ borderRadius: 2, overflow: "hidden", background: "var(--card)", border: "1px solid var(--border)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderBottom: "1px solid var(--border)" }}>
                <div style={{ fontSize: 10, fontWeight: 600, padding: "3px 9px", borderRadius: 1, letterSpacing: "0.08em", textTransform: "uppercase", background: sc.bg, color: sc.color }}>{t(b.status)}</div>
              ...
              <div style={{ display: "flex", gap: 7, padding: "0 14px 12px" }}>
                <div style={{ flex: 1, padding: 8, fontSize: 11, fontWeight: 500, textAlign: "center", borderRadius: 2, background: "var(--dim)", color: "var(--muted)" }}>{b.price} €</div>
                {["confirmed","pending"].includes(b.status) && (
                  <button onClick={() => goBook()} style={{ flex: 1, padding: 8, fontSize: 11, fontWeight: 500, textAlign: "center", borderRadius: 2, border: "1px solid var(--border)", background: "var(--gold-l)", color: "var(--gold)", cursor: "pointer" }}>{t("reschedule")}</button>
                )}
              </div>
            </div>
          );
  ...
  if (screen === "ai") return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, background: "var(--card)", borderBottom: "1px solid var(--border)", position: "relative" }}>
        ...
        <div>
          <div className="serif" style={{ fontSize: 15, fontWeight: 600, color: "var(--fg)" }}>{t("ai")}</div>
          <div style={{ fontSize: 11, marginTop: 1, color: "var(--muted)" }}>{t("aiSub")}</div>
        </div>
        <div style={{ marginLeft: "auto", fontSize: 9, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", padding: "3px 8px", border: "1px solid rgba(201,168,76,.3)", color: "var(--gold)", borderRadius: 1, marginRight: 40 }}>Beta</div>
        <ThemeBtn />
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
        {aiMessages.length === 0 && (
          <div style={{ alignSelf: "flex-start", maxWidth: "85%" }}>
            <div style={{ padding: "10px 13px", fontSize: 12, lineHeight: 1.6, borderRadius: 2, borderBottomLeftRadius: 0, background: "var(--card)", color: "var(--fg)", border: "1px solid var(--border)" }}>{t("aiGreeting")}</div>
        ...
        {aiMessages.map((m, i) => (
          <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%" }}>
            <div style={{
              ...
              border: m.role === "user" ? "none" : "1px solid var(--border)", fontWeight: m.role === "user" ? 500 : 400,
            }}>{m.content}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "var(--card)", borderTop: "1px solid var(--border)" }}>
        <input value={aiInput} onChange={e => setAiInput(e.target.value)}
          ...
          style={{ flex: 1, borderRadius: 2, padding: "9px 13px", fontSize: 12, background: "var(--dim)", color: "var(--fg)", border: "1px solid var(--border)", outline: "none" }}
        />
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 12px", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ borderRadius: 2, padding: 16, background: "var(--card)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 12 }}>{t("about")}</div>
          ...
          {salon.description && <div style={{ paddingTop: 8, fontSize: 12, lineHeight: 1.6, color: "var(--fg)" }}>{salon.description}</div>}
          {!salon.name       && <div style={{ fontSize: 12, color: "var(--muted)", textAlign: "center", padding: "20px 0" }}>Hunger Beauty Salon</div>}
        </div>
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div style={{ padding: "20px 16px 18px", display: "flex", alignItems: "center", gap: 16, background: "var(--card)", borderBottom: "1px solid var(--border)", position: "relative" }}>
        ...
        <div style={{
          ...
          fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 600, flexShrink: 0,
          boxShadow: "0 0 18px var(--gold-g)",
        }}>{initLetter}</div>
        <div style={{ paddingRight: 44 }}>
          <div className="serif" style={{ fontSize: 19, fontWeight: 600, color: "var(--fg)" }}>{firstName}{user?.last_name ? ` ${user.last_name}` : ""}</div>
          {user?.username && <div style={{ fontSize: 12, marginTop: 3, color: "var(--muted)" }}>@{user.username}</div>}
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 5, padding: "2px 8px", borderRadius: 1, display: "inline-block", background: "var(--gold-l)", color: "var(--gold)", border: "1px solid var(--border)" }}>Telegram</div>
        </div>
        <ThemeBtn />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", background: "var(--card)", borderBottom: "1px solid var(--border)" }}>
        {[{val: upcoming.length, k: t("upcoming")}, {val: past.length, k: t("visits")}, {val: lang.toUpperCase(), k: t("language")}].map(({val, k}, i) => (
          <div key={k} style={{ padding: "12px 8px", textAlign: "center", borderRight: i < 2 ? "1px solid var(--border)" : undefined }}>
            <div className="serif" style={{ fontSize: 20, fontWeight: 600, color: "var(--fg)" }}>{val}</div>
            <div style={{ fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 3, color: "var(--muted)" }}>{k}</div>
          </div>
        ))}
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
        {[
          { icon: IC.list,     label: t("myBookings"), action: () => { haptic(); setScreen("bookings"); } },
          { icon: IC.book,     label: t("book"),       action: () => goBook() },
          { icon: IC.settings, label: t("settings"),   action: () => { haptic(); setScreen("settings"); } },
        ].map(({icon, label, action}) => (
          <button key={label} onClick={action} style={{
            display: "flex", alignItems: "center", gap: 12, padding: "12px 14px",
            borderRadius: 2, cursor: "pointer", background: "var(--card)", border: "1px solid var(--border)",
  ...
  if (screen === "settings") return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: "14px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Theme */}
        <div style={{ borderRadius: 2, padding: 16, background: "var(--card)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 12 }}>{t("themeTitle")}</div>
          <div style={{ display: "flex", gap: 8 }}>
            {(["dark","light"] as Theme[]).map(th => (
              <button key={th} onClick={() => { haptic(); setTheme(th); }} style={{
                flex: 1, padding: "10px 8px", borderRadius: 2, cursor: "pointer",
                ...
                <span style={{ fontSize: 12, fontWeight: theme === th ? 600 : 400, color: theme === th ? "var(--gold)" : "var(--muted)", letterSpacing: "0.06em", textTransform: "uppercase" }}>{t(th)}</span>
                {theme === th && <span style={{ marginLeft: "auto", background: "var(--gold)", borderRadius: "50%", width: 18, height: 18, display: "grid", placeItems: "center" }}>{IC.check}</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Language */}
        <div style={{ borderRadius: 2, padding: 16, background: "var(--card)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 12 }}>{t("langTitle")}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {([["ru","Русский","🇷🇺"],["uk","Українська","🇺🇦"],["en","English","🇬🇧"],["bg","Български","🇧🇬"]] as [Lang,string,string][]).map(([code, label, flag]) => (
              <button key={code} onClick={() => { haptic(); setLang(code); }} style={{
                ...
                <span style={{ fontSize: 12, fontWeight: lang === code ? 600 : 400, color: lang === code ? "var(--gold)" : "var(--muted)" }}>{label}</span>
                {lang === code && <span style={{ marginLeft: "auto", background: "var(--gold)", borderRadius: "50%", width: 18, height: 18, display: "grid", placeItems: "center", flexShrink: 0 }}>{IC.check}</span>}
              </button>
            ))}
          </div>
        </div>
