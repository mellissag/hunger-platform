...
export function MasterServices({ masterId }: { masterId: string }) {
  ...
  if (loadingAll || loadingMaster) return <div className="p-8 text-center text-muted-foreground">{t("loading")}</div>;

  const byCategory: Record<string, Service[]> = {};
  for (const s of services) {
    const cat = s.category?.name_i18n?.[locale] || s.category?.name_i18n?.en || s.category?.name || t("otherCategory");
    byCategory[cat] ??= [];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold">{t("masterServicesTitle")}</h3>
          <p className="text-sm text-muted-foreground">
            {t("masterServicesSelected", { enabled: enabledCount, total: services.length })}
          </p>
        </div>
            ...
            return (
              <div key={service.id} className={`rounded-md border p-3 ${enabled ? "border-primary/40 bg-accent/20" : ""}`}>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="flex-1 font-medium">{service.name_i18n?.[locale] || service.name_i18n?.en || service.name || t("serviceFallback")}</span>
                  <span className="text-xs text-muted-foreground">
                    €{service.price} · {service.duration_minutes} {t("minutesShort")}
                  </span>
                </div>
                {enabled ? (
